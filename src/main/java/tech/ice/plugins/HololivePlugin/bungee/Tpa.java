...
public class Tpa extends Command implements TabExecutor {
    ...
    public void execute(CommandSender sender, String[] args) {
        if (args.length != 1) {
            TextComponent text = new TextComponent("用法：/tpa <player>");
        ...
        if (progress.get(ProxyServer.getInstance().getPlayer(args[0])) != null) {
            if (progress.get(ProxyServer.getInstance().getPlayer(args[0])).equals(new AbstractMap.SimpleEntry<>((ProxiedPlayer) sender, "tpa"))) {
        ...
        BaseComponent[] receive = new ComponentBuilder(sender.getName()).color(ChatColor.AQUA)
                .append(" 請求傳送到你這裡").color(ChatColor.GOLD).create();
